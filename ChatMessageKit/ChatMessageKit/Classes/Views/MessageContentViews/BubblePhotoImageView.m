...
- (void)configureMessagePhoto:(UIImage *)messagePhoto thumbnailUrl:(NSString *)thumbnailUrl originPhotoUrl:(NSString *)originPhotoUrl onBubbleMessageType:(BubbleMessageType)bubbleMessageType {
    ...
    if (thumbnailUrl) {
        ...
        NSString *placeholderImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageTablePlaceholderImageNameKey];
        if (!placeholderImageName) {
            placeholderImageName = @"placeholderImage";
        }
        
        self.messagePhoto = [UIImage imageNamed:placeholderImageName];
        [[YYWebImageManager sharedManager]requestImageWithURL:[NSURL URLWithString:thumbnailUrl] options:YYWebImageOptionUseNSURLCache progress:nil transform:nil completion:^(UIImage *image,NSURL *url,YYWebImageFromType from,YYWebImageStage stage,NSError *error){
