...
- (UIImage *)getAvatarPlaceholderImage {
    NSString *avatarPalceholderImageName = [[ConfigurationHelper appearance].messageTableStyle objectForKey:kMessageTableAvatarPalceholderImageNameKey];
...
- (void)configAvatarWithPhotoURLString:(NSString *)photoURLString {
    [self.avatarButton setImageWithURL:[NSURL URLWithString:photoURLString] forState:UIControlStateNormal placeholder:[self getAvatarPlaceholderImage]];
...
- (instancetype)initWithMessage:(id <MessageModel>)message
    ...
    if (self) {
        ...
        // 1、是否显示Time Line的label
        if (!_timestampLabel) {
            UIColor *timestampLabelTextColor = [[ConfigurationHelper appearance].messageTableStyle objectForKey:kMessageTableTimestampTextColorKey];
            if (!timestampLabelTextColor) {
                timestampLabelTextColor = [UIColor whiteColor];
            }
            UIColor *timestampBackgroundColor = [[ConfigurationHelper appearance].messageTableStyle objectForKey:kMessageTableTimestampBackgroundColorKey];
            if (!timestampBackgroundColor) {
                timestampBackgroundColor = [UIColor colorWithWhite:0.734 alpha:1.000];
            }
            LKBadgeView *timestampLabel = [[LKBadgeView alloc] initWithFrame:CGRectMake(0, kLabelPadding, MDK_SCREEN_WIDTH, kTimeStampLabelHeight)];
