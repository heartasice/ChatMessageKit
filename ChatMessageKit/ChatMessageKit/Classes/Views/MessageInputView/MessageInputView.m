...
- (void)setupMessageInputViewBarWithStyle:(MessageInputViewStyle)style {
    ...
    if (self.allowsSendVoice) {
        NSString *voiceNormalImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewVoiceNormalImageNameKey];
        if (!voiceNormalImageName) {
            voiceNormalImageName = @"voice";
        }
        NSString *voiceHLImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewVoiceHLImageNameKey];
        if (!voiceHLImageName) {
            voiceHLImageName = @"voice_HL";
        }
        NSString *keyboardNormalImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewKeyboardNormalImageNameKey];
        if (!keyboardNormalImageName) {
            keyboardNormalImageName = @"keyboard";
        }
        NSString *keyboardHLImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewKeyboardHLImageNameKey];
    ...
    if (self.allowsSendMultiMedia) {
        NSString *extensionNormalImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewExtensionNormalImageNameKey];
        if (!extensionNormalImageName) {
            extensionNormalImageName = @"multiMedia";
        }
        NSString *extensionHLImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewExtensionHLImageNameKey];
    ...
    if (self.allowsSendFace) {
        NSString *emotionNormalImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewEmotionNormalImageNameKey];
        if (!emotionNormalImageName) {
            emotionNormalImageName = @"face";
        }
        NSString *emotionHLImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewEmotionHLImageNameKey];
    ...
    UIColor *placeHolderTextColor = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewPlaceHolderTextColorKey];
    if (placeHolderTextColor) {
        textView.placeHolderTextColor = placeHolderTextColor;
    }
    
    UIColor *textColor = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewTextColorKey];
    if (textColor) {
        textView.textColor = textColor;
    }
    
    NSString *placeHolder = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewPlaceHolderKey];
    if (!placeHolder) {
    ...
    switch (style) {
        case MessageInputViewStyleFlat: {
            UIColor *inputBackgroundColor = [ConfigurationHelper appearance].messageInputViewStyle[kMessageInputViewBackgroundColorKey];
            NSString *inputViewBackgroundImageName = nil;
            if (!inputBackgroundColor) {
                inputViewBackgroundImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewBackgroundImageNameKey];
                if (!inputViewBackgroundImageName) {
                    inputViewBackgroundImageName = @"input-bar-flat";
                }
            }
            
            UIColor *borderColor = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewBorderColorKey];
            if (!borderColor) {
                borderColor = [UIColor colorWithWhite:0.8f alpha:1.0f];
            }
            CGFloat borderWidth = [[[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewBorderWidthKey] floatValue];
            if (borderWidth == 0) {
                borderWidth = 0.65f;
            }
            CGFloat cornerRadius = [[[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewCornerRadiusKey] floatValue];
            if (cornerRadius == 0) {
                cornerRadius = 6.0f;
            }
    ...
    if (self.allowsSendVoice) {
        NSString *voiceHolderImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewVoiceHolderImageNameKey];
        if (!voiceHolderImageName) {
            voiceHolderImageName = @"VoiceBtn_Black";
        }
        NSString *voiceHolderHLImageName = [[ConfigurationHelper appearance].messageInputViewStyle objectForKey:kMessageInputViewVoiceHolderHLImageNameKey];
        if (!voiceHolderHLImageName) {
            voiceHolderHLImageName = @"VoiceBtn_BlackHL";
        }
        
        UIEdgeInsets edgeInsets = UIEdgeInsetsMake(9, 9, 9, 9);
        button = [self createButtonWithImage:STRETCH_IMAGE([UIImage imageNamed:voiceHolderImageName], edgeInsets) HLImage:STRETCH_IMAGE([UIImage imageNamed:voiceHolderHLImageName], edgeInsets)];
