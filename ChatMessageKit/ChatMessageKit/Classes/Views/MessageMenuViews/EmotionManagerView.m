...
#import "EmotionSectionBar.h"

#import "EmotionCollectionViewCell.h"
#import "EmotionCollectionViewFlowLayout.h"


@interface EmotionManagerView () <UICollectionViewDelegate, UICollectionViewDataSource, EmotionSectionBarDelegate>
 ...
 *  管理多种类别gif表情的滚动试图
 */
@property (nonatomic, weak) EmotionSectionBar *emotionSectionBar;
...
@implementation EmotionManagerView
...
- (void)reloadData {
    ...
    EmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:self.selectedIndex];
    NSInteger numberOfEmotions = emotionManager.emotions.count;
    self.emotionPageControl.numberOfPages = (numberOfEmotions / (kEmotionPerRowItemCount * 2) + (numberOfEmotions % (kEmotionPerRowItemCount * 2) ? 1 : 0));
...
- (void)setup {
    ...
    if (!_emotionCollectionView) {
        UICollectionView *emotionCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kEmotionPageControlHeight - kEmotionSectionBarHeight) collectionViewLayout:[[EmotionCollectionViewFlowLayout alloc] init]];
        emotionCollectionView.backgroundColor = self.backgroundColor;
        [emotionCollectionView registerClass:[EmotionCollectionViewCell class] forCellWithReuseIdentifier:kEmotionCollectionViewCellIdentifier];
    ...
    if (!_emotionPageControl) {
        UIPageControl *emotionPageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.emotionCollectionView.frame), CGRectGetWidth(self.bounds), kEmotionPageControlHeight)];
    ...
    if (!_emotionSectionBar) {
        EmotionSectionBar *emotionSectionBar = [[EmotionSectionBar alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.emotionPageControl.frame), CGRectGetWidth(self.bounds), kEmotionSectionBarHeight) showEmotionStoreButton:self.isShowEmotionStoreButton];
...
#pragma mark - EmotionSectionBar Delegate
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    EmotionCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kEmotionCollectionViewCellIdentifier forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(didSelecteEmotion:atIndexPath:)]) {
        EmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:indexPath.section];
