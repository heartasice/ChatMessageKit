...
#define kStoreManagerItemWidth 40

@interface EmotionSectionBar ()
...
@implementation EmotionSectionBar
...
- (UIButton *)cratedButton {
    ...
    button.frame = CGRectMake(0, 0, kStoreManagerItemWidth, CGRectGetHeight(self.bounds));
...
- (void)reloadData {
    ...
    for (EmotionManager *emotionManager in self.emotionManagers) {
    ...
    [self.sectionBarScrollView setContentSize:CGSizeMake(self.emotionManagers.count * kStoreManagerItemWidth, CGRectGetHeight(self.bounds))];
}
...
- (void)setup {
    if (!_sectionBarScrollView) {
        ...
        if (self.isShowEmotionStoreButton) {
            scrollWidth -= kStoreManagerItemWidth;
        }
    ...
    if (self.isShowEmotionStoreButton) {
        ...
        storeManagerItemButtonFrame.origin.x = CGRectGetWidth(self.bounds) - kStoreManagerItemWidth;
